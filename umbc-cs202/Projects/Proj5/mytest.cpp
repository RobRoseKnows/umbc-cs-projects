...
int main() {
    ...
    cout << "Inserted what: " << *(x.insert(21)) << endl;
    cout << "Inserted what: " << *(x.insert(-3)) << endl;
    cout << "Inserted what: " << *(x.insert(9)) << endl;
    ...
    // BIG IMPORTANT NOTE HERE: RAND_ITERATOR HAS TO BE DECLARED WITH
    // THE SORTED LIST IF YOU DON'T WANT IT TO THROW AN EXCEPTION
    // BECAUSE RAND_ITERATOR WILL INITIALIZE WITH A DIFFERENT VERSION #
    sorted::rand_iterator rnditr(&x);
    ...
    try {
        ...
        for (rnditr = x.rndbegin(11111); rnditr != x.rndend(); rnditr++)
            cout << *rnditr << " ";
        cout << endl << endl;
    } catch(exception e) {
        cerr << e.what();
...
//    for (rndsongitr = songs.rndbegin(); rndsongitr != songs.rndend();
//            rndsongitr++)
