...
sorted::rand_iterator::rand_iterator(sorted* srtdPtr) {
    ...
    m_seed = 12345;
    m_sorted = srtdPtr;
    m_rand = new int[m_sorted->m_size];
...
sorted::rand_iterator::rand_iterator(const rand_iterator& itr) {
    ...
    m_rand = new int[itr.m_sorted->m_size];
...
//                "sorted::rand_iterator::operator++() underlying array has changed");
    ...
    if (m_sorted->m_version != m_version_created) {
        cerr << "sorted: " << m_sorted->m_version << " this: " << m_version_created;
        throw iterator_ex("sorted::rand_iterator::operator++() underlying array has changed");
    }

    // TODO: THE PROBLEM IS THAT ITERATING THROUGH m_current THIS WAY
    // WILL JUST ITERATE THROUGH IT IN SORTED ORDER. I NEED TO MAKE AN
    // ARRAY WITH THE VALUES, NOT JUST THE INDEXES.
    ++m_current;
    return *this;
}
...
sorted::rand_iterator sorted::rand_iterator::operator++(int) {
    if (m_sorted->m_version != m_version_created) {
        cerr << "sorted: " << m_sorted->m_version << " this: " << m_version_created;
        throw iterator_ex("sorted::rand_iterator::operator++(int) underlying array has changed");
    }
...
//                "sorted::rand_iterator::operator!=(rand_iterator&) second array has changed");
    ...
    if (m_sorted->m_version != m_version_created) {
        cerr << "sorted: " << m_sorted->m_version << " this: " << m_version_created;
        throw iterator_ex("sorted::rand_iterator::operator!=(rand_iterator&) first array has changed");
    }

    if (itr.m_sorted->m_version != itr.m_version_created) {
        cerr << "sorted: " << itr.m_sorted->m_version << " this: " << itr.m_version_created;
        throw iterator_ex("sorted::rand_iterator::operator!=(rand_iterator&) second array has changed");
    }
