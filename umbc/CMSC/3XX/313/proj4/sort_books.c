...
int main(int argc, char **argv) {
    ...
    for (i = 0; i < MAX_BOOKS; i++) {
        ...
        char* subjectPtr = currBook->subject;
        unsigned int* yearPtr = &(currBook->year);

        int numFields = scanf("%80[^,], %20[^,], %10[^,], %u \n",
                titleBuffer, authorPtr, subjectPtr, yearPtr);
    ...
    for (i = 0; i < numBooks - 1; i++) {
        ...
        for (j = i + 1; j < numBooks; j++) {
            ...
            * it into cmpResult */
             ...
             * (cmpResult) is one, it means it's greater than.
             */
            if (cmpResult == 1) {
                min = j;
            }
...
void print_books(struct book books[], int numBooks) {
    ...
    for(i = 0; i < numBooks; i++) {
        ...
        printf("%s, %s, %s, %d", books[i].title, books[i].author, books[i].subject, books[i].year);
